...
<div class="">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h2 class="card-title"><strong>Form Data Kelas</strong></h2>
            ...
            <form action="{{ route('kelas.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="jurusan" name="jurusan" required>
                        ...
                        <option value="Geospasial" {{ old('jurusan') == 'Geospasial' ? 'selected' : '' }}>Geospasial</option>
                        <option value="MPLB" {{ old('jurusan') == 'MPLB' ? 'selected' : '' }}>MPLB</option>
                        <option value="PPLG" {{ old('jurusan') == 'PPLG' ? 'selected' : '' }}>PPLG</option>
                        <option value="TAV" {{ old('jurusan') == 'TAV' ? 'selected' : '' }}>TAV</option>
                        <option value="Teknik Kendaraan Ringan" {{ old('jurusan') == 'Teknik Kendaraan Ringan' ? 'selected' : '' }}>Teknik Kendaraan Ringan</option>
                        <option value="Teknik Las" {{ old('jurusan') == 'Teknik Las' ? 'selected' : '' }}>Teknik Las</option>
                        <option value="Teknik Mesin" {{ old('jurusan') == 'Teknik Mesin' ? 'selected' : '' }}>Teknik Mesin</option>
                        <option value="Teknik Sepeda Motor" {{ old('jurusan') == 'Teknik Sepeda Motor' ? 'selected' : '' }}>Teknik Sepeda Motor</option>
                        <option value="TKJT" {{ old('jurusan') == 'TKJT' ? 'selected' : '' }}>TKJT</option>
                    </select>
                </div>
